#!/usr/bin/env python3
"""Small PR-AS freeform observe-relevance probe. Diagnostic only."""
...
STOCK_A = "threadbare watchers"
STOCK_B = "gate serjeant"
...
def main() -> int:
    ...
    lines = [
        ...
        "I listen.",
        "I step back from the board and wait.",
    ]
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    out_dir = ROOT / "artifacts" / "pras_observe_fallback_relevance" / "freeform_probe"
            ...
            turns.append(
                {
                    ...
                    "stock_watchers": STOCK_A in low,
                    "stock_serjeant": STOCK_B in low,
                    "invented_whisper": "whisper" in low,
                    "nothing_new": "nothing new stands out" in low,
                }
            )
    report = {"stamp": stamp, "turns": turns}
    (out_dir / f"{stamp}_probe.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    md_lines = [f"# PR-AS freeform observe-relevance probe {stamp}", ""]
    for i, turn in enumerate(turns, 1):
        md_lines.extend(
            [
                ...
                (
                    f"- stock watchers/serjeant: `{turn['stock_watchers']}` / `{turn['stock_serjeant']}`"
                ),
                f"- invented whisper: `{turn['invented_whisper']}` nothing-new: `{turn['nothing_new']}`",
                f"- leads: `{turn['leads']}`",
                "",
            ]
        )
