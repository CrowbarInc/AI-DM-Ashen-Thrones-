...
def render_migration_report_markdown(result: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"| Session diagnostic history | {int(summary.get('diagnostic_event_count') or 0)} |",
        "",
...
def migrate_bug_recurrence_event_log(
    *,
    ...
    """Split a unified recurrence event log into protected and session-diagnostic lanes."""
    log_path = Path(event_log_path)
    session_log_path = Path(session_diagnostic_event_log_path)
    ...
    diagnostic_events = list(classification["session_diagnostic_history"])

    protected_log = _event_log_envelope(protected_events)
    diagnostic_log = _event_log_envelope(diagnostic_events)
    ...
    if not dry_run and not report_only:
        ...
        write_recurrence_event_log(log_path, protected_log)
        write_recurrence_event_log(session_log_path, diagnostic_log)
        write_bug_recurrence_history_artifacts(
            None,
        ...
        metrics_regenerated = history_json.is_file() and history_md.is_file()
    ...
    result: dict[str, Any] = {
        ...
        "session_diagnostic_event_log_path": str(session_log_path.as_posix()),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Session diagnostic recurrence event log output path.",
    )
    parser.add_argument(
        "--history-json",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Diagnostic events: {summary['diagnostic_event_count']}")
    print(
