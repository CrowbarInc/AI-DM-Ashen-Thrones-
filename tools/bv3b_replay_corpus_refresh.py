#!/usr/bin/env python3
"""BV3B — regenerate canonical finalized-FEM artifact corpus under current gate code."""
...
SESSION_LOG_BACKUP = ROOT / "artifacts" / "bv3b_replay_refresh" / "session_log.pre_refresh.jsonl"
REFRESH_MANIFEST = ROOT / "artifacts" / "bv3b_replay_refresh" / "corpus_refresh_manifest.json"
OBSERVE_PROMPTS = (
    "I look around.",
...
def refresh_session_log(*, hygiene_batches: int) -> dict:
    backup_dir = SESSION_LOG_BACKUP.parent
    backup_dir.mkdir(parents=True, exist_ok=True)
    src = ROOT / "data" / "session_log.jsonl"
    if src.is_file():
        shutil.copy2(src, SESSION_LOG_BACKUP)

    archive_hygiene = backup_dir / f"scene_canon_hygiene_runtime.{_utc_now().replace(':', '')}"
    ...
    # Promote the richest observe run into canonical data/session_log.jsonl.
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest: dict = {"schema_version": 1, "generated_at": _utc_now(), "steps": []}
