...
- **Sealed replacement lineage sub-kinds (Cycle AB6).** Non-strict terminal replace turns still share write-time `gate_terminal_repair` and distinct `final_emitted_source` values. Read-side projection in `game/final_emission_replay_projection.py` maps those sources to stable `sealed_*` lineage `fallback_kind` sub-kinds (replacing the catch-all `sealed_or_global_replacement` bucket on `fem_runtime_lineage_events` only). Runtime FEM and protected golden replay fields are unchanged.
