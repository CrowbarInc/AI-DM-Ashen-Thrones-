...
- **Objective N5 (`clause_referent_plan` → `referent_clause_prompt_hints`):** Optional `clause_referent_plan` on the referent-tracking artifact is **derivative-only** structure built only in `game.referent_tracking`. This adapter may attach the trimmed **`referent_clause_prompt_hints`** list when that field exists; it must **not** synthesize a parallel referent artifact, infer clause rows from model prose, or treat N5 rows as CTIR meaning. CTIR remains the sole resolved-turn semantic authority (`docs/clause_level_referent_tracking.md`, `tests/test_n5_boundary_regressions.py`).
