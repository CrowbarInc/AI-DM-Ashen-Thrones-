...
| **Test Ownership & Coverage — Block D (inventory schema v2; AQ governance split)** | Pytest-only: committed `tests/test_inventory_governance.json`, registry derived from Python, governance tests + `--check` in CI | `tools/test_audit.py`, `tests/test_ownership_registry.py`, `tests/README_TESTS.md`, `tests/TEST_AUDIT.md`, `tests/TEST_CONSOLIDATION_PLAN.md`, `docs/validation_layer_separation.md` |
