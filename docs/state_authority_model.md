...
3. **Prompt packaging:** `game/prompt_context.build_narration_context` attaches `narration_visibility`, `scene_state_anchor_contract`, and `scene.gm_only` slices for the model to **read** under rules; those bundles are instruction carriers, not alternate stores for `world_state` or `hidden_state`. Optional **N5** `clause_referent_plan` on `referent_tracking` (and the trimmed `referent_clause_prompt_hints` projection) is the same class of **read-side derivative**: owned and constructed only in `game/referent_tracking.py`, not a parser, planner, or truth store, and not a substitute for CTIR (`docs/clause_level_referent_tracking.md`).
