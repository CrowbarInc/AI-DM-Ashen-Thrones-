...
def test_escalation_second_press_partial_not_exhausted():
    session = session_with_pressure("scene_investigate", "crossroads_incident", "runner", 2)
...
def test_escalation_third_press_actionable_and_conditioned_refusal():
    session = session_with_pressure("scene_investigate", "crossroads_incident", "runner", 3)
    out = determine_social_escalation_outcome(
        session=session,
        scene_id="scene_investigate",
        npc_id="runner",
...
def test_escalation_fourth_press_suspicion_and_momentum():
    session = session_with_pressure("scene_investigate", "crossroads_incident", "runner", 4)
    out = determine_social_escalation_outcome(
        session=session,
        scene_id="scene_investigate",
        npc_id="runner",
...
def test_answer_pressure_details_semantic_escalation_reason_at_repeat_one():
    session = session_with_pressure("scene_investigate", "crossroads_incident", "runner", 1)
    ap = {
...
def test_excessive_grinding_triggers_suspicion_flag():
    session = session_with_pressure("scene_investigate", "crossroads_incident", "runner", 5)
    out = determine_social_escalation_outcome(
        session=session,
        scene_id="scene_investigate",
        npc_id="runner",
...
def test_regression_runner_followup_sets_strategy_forced_to_answer():
    ...
    session = session_with_pressure("scene_investigate", "tavern_rumors", "runner", 2)
    from game.storage import get_scene_runtime
