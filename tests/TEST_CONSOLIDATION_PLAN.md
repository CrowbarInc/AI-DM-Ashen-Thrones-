...
**Prompt-contract governance note (post-PC2):** Treat `game/prompt_context.py` as the canonical runtime owner and `tests/test_prompt_context.py` as the practical primary direct-owner suite for prompt-contract semantics. `tests/test_prompt_compression.py` remains secondary integration coverage, while `tests/test_final_emission_gate.py`, `tests/test_social_exchange_emission.py`, `tests/test_narration_transcript_regressions.py`, and `tests/test_prompt_and_guard.py` are downstream consumers or prompt-adjacent evidence rather than co-equal owners.
