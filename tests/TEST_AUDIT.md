# Test suite audit (Block 15A + Test Ownership Block D)

Diagnostic inventory of `tests/` only. **How to run tests (fast/full lanes, collect-only, Windows):** `tests/README_TESTS.md`.

**Consolidation phase:** Behavioral Gauntlet, Playability Validation, and AER are **complete** as validation tracks. Emit-path **ownership** for prompt, response-policy, gate, repairs, telemetry, and strict-social emission is **governance-resolved** (singular runtime owner + practical direct-owner suite + intentional downstream consumers per `docs/architecture_ownership_ledger.md`). **Test Ownership Blocks A–D** add **inventory schema v2** and **`tests/test_ownership_registry.py`** so the map is machine-checked and resistant to drift. Optional follow-ups: **transcript** duplicate assertion thinning, deferred **lead/clue** batch, and marker tidy—**without** expanding gameplay behavior in doc-only passes. Runtime **orchestration** layout remains in `docs/narrative_integrity_architecture.md`.
...
**Block C final split (strict-social vs prompt vs gate):** **`tests/test_social_exchange_emission.py`** owns strict-social **first-sentence** and **question-resolution** legality (including `question_resolution_rule_check` direct tables where applicable). **`tests/test_prompt_and_guard.py`** owns **`build_retry_prompt_for_failure`** copy for unresolved-question / social-contract failures, **`enforce_question_resolution_rule` prepend** behavior, **validator-voice** detect/rewrite at the GM layer, and **thin smoke** adjacent to the prompt stack for that boundary. **`tests/test_final_emission_gate.py`** owns **`apply_final_emission_gate` orchestration** (layer order, final-route integration).

**Governance pytest:** after regenerating or editing inventory-sensitive paths, run `py -3 -m pytest tests/test_ownership_registry.py -q`. That module encodes required **direct-owner** groups and allowlisted duplicate-name reasons; it complements the audit script and does **not** replace semantic code review. **Split-owner matrix edits:** start at [`docs/convergence_ci_inventory.md`](../docs/convergence_ci_inventory.md) → *Split-owner acceptance matrix governance*; edit checklist in [`docs/audits/README.md`](../docs/audits/README.md). Broader convergence CI slices: same inventory (workflow: `.github/workflows/convergence-checks.yml`).
...
primary direct-owner suite — including **first-sentence** and **question-resolution** legality tables
(`question_resolution_rule_check` and related strict-social string contracts). **`tests/test_prompt_and_guard.py`**
carries **retry prompt text** for unresolved-question / social-contract failures, **`enforce_question_resolution_rule` prepend**,
and **validator-voice** GM-layer checks, with **only smoke** overlap for the strict-social legality boundary (no second full direct matrix there). Keep `tests/test_strict_social_emergency_fallback_dialogue.py` as
